...
- **16GB policy (one place).** `master_agent/models/vram_policy.py` — doctor, catalog, patcher, and `--prepare` inherit it. **GGUF first, then NVFP4.** `--quality 16gb` is a first-class profile. Large Mickmumpitz graphs (Movie Builder, CCC) are labeled `needs_more_vram` with a documented alternate — do not claim they fit.
- **LTX 2.5 default catalog (PR #6, merged).** `ltx25_*` variants (and research aliases `t2v_i2v` / `flf2v` / …) are listed with no env flag. Inventory first. 16GB-class loader pick: GGUF Q4 → NVFP4 (if `VRAM_GB` ≥ 14) → int8-convrot → bf16. Heretic/int8 TE counts; official bf16 Gemma is not required. Zero-byte duration-head = missing. `doctor` reports the pick and does **not** fetch. `download-models --ltx25` lists confirmed-missing; `--yes` only after the ask. Baked graphs are 768×512 / 25f (two-stage 17f).
...
- **Wan / VACE / Krea on 16GB.** Wan 2.2 T2V has **no catalogued GGUF** — default is dual fp8 14B + LightX2V at 640×384 / ≤33f (offload). VACE / AI-VFX defaults to Skyreels **GGUF Q4_K_M**. Krea turbo defaults to **NVFP4** at 1024×576. K3NK AIO I2V is not shipped.
...
.\.venv\Scripts\python.exe -m pytest tests/test_ltx25_catalog.py tests/test_ltx25_weights.py tests/test_h3_catalog.py tests/test_h3_weights.py tests/test_capabilities.py -q
```
